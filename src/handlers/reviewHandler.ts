...
function buildReviewMessage(
    ...
    logger.debug('buildReviewMessage called', {
        ...
        hasPendingText: !!pendingResponseText,
    });
    ...
    message += `👤 ${escapeHtml(reviewerName)}${territory}\n`;

    if (review.review_date) {
        message += `📅 ${formatReviewDate(review.review_date)}\n`;
    }
...
function buildReviewKeyboard(reviewId: string, hasResponse: boolean) {
    if (!hasResponse) {
        return Markup.inlineKeyboard([
            [Markup.button.callback('🔄 Generate Response', `generate_${reviewId}`)],
            [Markup.button.callback('❌ Dismiss', `dismiss_${reviewId}`)],
        ]);
    }

    return Markup.inlineKeyboard([
        [
            Markup.button.callback('✅ Approve', `approve_${reviewId}`),
            Markup.button.callback('✏️ Edit', `edit_${reviewId}`),
        ],
        [
            Markup.button.callback('🔄 Regenerate', `regenerate_${reviewId}`),
            Markup.button.callback('❌ Reject', `reject_${reviewId}`),
        ],
    ]);
}
...
function buildIteratorKeyboard(reviewId: string, hasResponse: boolean, currentIndex: number, total: number) {
    ...
    if (!hasResponse) {
        rows.push([
            Markup.button.callback('🤖 Generate AI Response', `iter_generate_${reviewId}`),
        ]);
        rows.push([
            Markup.button.callback('✏️ Write Response', `iter_write_${reviewId}`),
        ]);
    } else {
        rows.push([
            Markup.button.callback('✅ Approve', `iter_approve_${reviewId}`),
            Markup.button.callback('✏️ Edit', `iter_edit_${reviewId}`),
        ]);
        rows.push([
            Markup.button.callback('🔄 Regenerate', `iter_regenerate_${reviewId}`),
            Markup.button.callback('❌ Reject', `iter_reject_${reviewId}`),
        ]);
    }
...
function buildPaginationButtons(currentIndex: number, total: number) {
    ...
    for (let page = startPage; page <= endPage; page++) {
        ...
        if (page === currentPage) {
            ...
            buttons.push(Markup.button.callback(`• ${page} •`, `iter_noop`));
        } else {
    ...
    if (endPage < total) {
        buttons.push(Markup.button.callback('»', `iter_goto_${total - 1}`));
    }
...
function buildIteratorMessage(
    ...
    let message = `📋 <b>Review ${currentIndex + 1} of ${total}</b>\n\n`;
...
export async function handleGenerateResponse(ctx: Context, reviewId: string): Promise<void> {
    ...
    if (!review) {
        ...
        await safeEditMessage(ctx, '❌ Review not found.');
        return;
    }
    ...
    if (!app) {
        ...
        await safeEditMessage(ctx, '❌ App not found.');
        return;
    }
    ...
    try {
        ...
        if (!user) {
            ...
            await safeEditMessage(ctx, '❌ User not found.');
            return;
        }

        const rateResult = generateLimiter.tryConsume(user.id);
        if (!rateResult.allowed) {
            const minutes = Math.max(1, Math.ceil((rateResult.resetAt - Date.now()) / 60000));
            logger.warn('Generate rate limit exceeded', { userId: user.id, telegramId: ctx.from!.id, limit: GENERATE_LIMIT_PER_HOUR });
            await safeEditMessage(ctx, `🚫 Rate limit reached. Please try again in about ${formatMinutesLabel(minutes)}.`);
            return;
        }

        logger.debug('Generating AI response', { reviewId, appName: app.name, rating: review.rating });

        // Immediately update the message to show generation is in progress
        const generatingMessage = buildReviewMessage(
            review,
            app,
            undefined,
            undefined,
            GENERATING_RESPONSE_TEXT
        );
        const generatingKeyboard = buildReviewKeyboard(review.id, false);

        await safeEditMessage(ctx, generatingMessage, {
            parse_mode: 'HTML',
            ...generatingKeyboard,
        });
        ...
        if (!response) {
            ...
            await safeEditMessage(ctx, '❌ Failed to save response. Please try again.');
            return;
        }
        ...
        await safeEditMessage(ctx, `❌ Failed to generate response: ${errorMessage}`);
    }
}
...
export async function handleApproveResponse(ctx: Context, reviewId: string): Promise<void> {
    ...
    if (!review) {
        logger.warn('Review not found', { reviewId });
        await ctx.editMessageText('❌ Review not found.');
        return;
    }
    ...
    if (!app) {
        logger.warn('App not found', { appId: review.app_id });
        await ctx.editMessageText('❌ App not found.');
        return;
    }

    if (!app.account || !app.account.is_valid) {
        logger.warn('Account invalid or missing', { appId: review.app_id, accountId: app.account_id });
        await ctx.editMessageText('❌ Account credentials are invalid. Please reconnect your account with /account.');
        return;
    }
    ...
    if (!response) {
        ...
        await ctx.editMessageText('❌ Response not found. Please generate one first.');
        return;
    }
    ...
    try {
        ...
        await safeEditMessage(ctx,
            ...
            `${'⭐'.repeat(review.rating)} by ${escapeHtml(review.reviewer_name || 'Anonymous')}\n\n` +
...
export async function handleEditResponse(ctx: Context, reviewId: string): Promise<void> {
    ...
    const response = await supabase.getResponseByReviewId(reviewId);
    if (!response) {
        await ctx.reply('❌ Response not found. Please generate one first.');
        return;
    }
    ...
    await ctx.reply(
        ...
        `<blockquote>${escapeHtml(customText)}</blockquote>`,
        {
            ...
            ...Markup.inlineKeyboard([
                [Markup.button.callback('✅ Approve & Post', `approve_${reviewId}`)],
                [Markup.button.callback('✏️ Edit Again', `edit_${reviewId}`)],
                [Markup.button.callback('❌ Cancel', `dismiss_${reviewId}`)],
            ]),
        }
    );
}
...
function buildAppFilterKeyboard(selectedAppIds: Set<string>, apps: { id: string; name: string; store: string }[]) {
    ...
    if (selectedAppIds.size === apps.length) {
        controlRow.push(Markup.button.callback('☐ Deselect All', 'filter_deselect_all'));
    } else {
        controlRow.push(Markup.button.callback('☑ Select All', 'filter_select_all'));
    }
    ...
    rows.push([
        Markup.button.callback(hasSelection ? '▶️ Continue' : '▶️ Show All', 'filter_continue'),
        Markup.button.callback('❌ Cancel', 'filter_cancel'),
    ]);
...
function buildAppFilterMessage(selectedAppIds: Set<string>, apps: { id: string; name: string; store: string }[]): string {
    let message = '🔍 <b>Filter Reviews by App</b>\n\n';
...
async function showIteratorReview(ctx: Context, telegramId: number, index: number, editMessage = false): Promise<void> {
    ...
    if (!state) {
        await ctx.reply('❌ Session expired. Please use /review to start again.');
        return;
    }
...
export async function handleIteratorGenerate(ctx: Context, reviewId: string): Promise<void> {
    ...
    try {
        ...
        if (!rateResult.allowed) {
            const minutes = Math.max(1, Math.ceil((rateResult.resetAt - Date.now()) / 60000));
            logger.warn('Iterator generate rate limit exceeded', { userId: user.id, telegramId: ctx.from.id, limit: GENERATE_LIMIT_PER_HOUR });
            await safeEditMessage(ctx, `🚫 Rate limit reached. Please try again in about ${formatMinutesLabel(minutes)}.`);
            return;
        }
        ...
        const total = state?.reviewIds.length ?? 1;

        const generatingMessage = buildIteratorMessage(
            review,
            app,
            currentIndex,
            total,
        ...
        const generatingKeyboard = buildIteratorKeyboard(review.id, false, currentIndex, total);
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        await ctx.editMessageText(`❌ Failed to generate response: ${errorMessage}`);
    }
}
...
export async function handleIteratorApprove(ctx: Context, reviewId: string): Promise<void> {
    ...
    try {
        ...
        // Show success
        await ctx.answerCbQuery('✅ Response posted!');
